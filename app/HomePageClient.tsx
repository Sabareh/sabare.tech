...
export default function HomePageClient({
  ...
  const mediumPosts = featuredPosts.filter((post) => post.source === "medium")
  const isMediumFeed = mediumPosts.length === featuredPosts.length && mediumPosts.length > 0
  const displayMediumCount = isMediumFeed
    ? typeof mediumPostCount === "number"
      ? mediumPostCount
      : mediumPosts.length
    : undefined
